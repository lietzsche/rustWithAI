...
이 과정은 완료되어 `archive/rust-basics/`에 보관되었고 완료 지점은 `rust-basics-complete` tag로 남아 있다. 14장까지 학습했다면 저장소 루트의 `CURRICULUM.md`에서 다음 프로젝트를 확인한다.

현재 후속 과정은 `projects/network-lab/`이다. byte와 blocking TCP부터 시작해 stream과 framing 문제를 경험한 뒤 concurrency, non-blocking I/O, async/Tokio로 확장한다.
