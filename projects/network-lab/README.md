...
- 각 구현 과제 전에 처음 사용하는 Rust 문법과 표준 라이브러리 API의 기본 형태를 완성 답안과 분리된 작은 예제로 먼저 확인한다.
- 기본 문법을 확인한 뒤 핵심 구현은 학습자가 직접 작성한다.
- 소단원을 완료하면 학습자가 명시적으로 `넘어가자`고 요청할 때까지 다음 단원을 시작하지 않는다.
